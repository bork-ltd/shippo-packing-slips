...
- `filter-order.ts` — `isTerminalOrderStatus(status)` — true for SHIPPED/PARTIALLY_FULFILLED/CANCELLED/REFUNDED (never print a slip for these, even with INCLUDE_ALL_ORDER_STATUSES)
